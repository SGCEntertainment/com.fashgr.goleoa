...
public class Engine : MonoBehaviour
{
	...
	const string gt3jkdfgk3 = "gt3jkdfgk3";
	const string wjkhoxa = "wjkhoxa";
	...
	IEnumerator SetupEncryptDataNoNull()
	{
		...
		{
            if (responce_from_server.Contains(gt3jkdfgk3))
            {
                OnFinalActionEvent?.Invoke(string.Empty);
                yield break;
            }
	...
	IEnumerator Get_First_Request(string uri)
	{
		...
		if(bot_tds_responce.Contains(gt3jkdfgk3))
		{
			OnFinalActionEvent?.Invoke(string.Empty);
			yield break;
        }
	...
	string GetTDSInfo(string beforeString, out bool geoContains)
	{
        ...
        bool IsNozim = geoArray.Contains(wjkhoxa);
